...
import { tenderDetail, tenderDetailProps } from "../constants/tenderInfo";
...
const tenderInfoFinalTable = (tenderInfo: any) => {
  let cost: number = 0;
  tenderInfo.purchases.map((item: { [index: string]: string }) => {
...
interface TenderTableProps {
  tenderInfoFinalTable: string[];
  tenderTableNames: string[];
  classes: any;
}

const cellNames: { [index: string]: string } = {
  inn: "INN:",
  name: "Tashkilot nomi:",
...
interface CompanyTenderTableProps {
  companyTable: { [index: string]: string };
  classes: any;
}
const CompanyTenderTable: FC<CompanyTenderTableProps> = ({
  companyTable,
  classes,
}) => {
  return (
    <Table size="small">
      <TableBody>
        {Object.entries(companyTable).map((item: string[]) => (
          <TableRow>
            <TableCell
              style={{ width: "50%" }}
...
const TenderTable: FC<TenderTableProps> = ({
  tenderInfoFinalTable,
  classes,
  tenderTableNames,
}) => {
  return (
    <Table className={classes.marginTable}>
      ...
      <TableBody>
        <TableRow>
          {tenderInfoFinalTable.map((item: string) => (
...
interface TenderTableStartProps {
  item: { [index: string]: string | number };
  classes: any;
}
const TenderTableStart: FC<TenderTableStartProps> = ({ item, classes }) => {
  return (
    <Table className={classes.marginTable}>
      <TableHead className={classes.headColor}>
        <TableRow>
